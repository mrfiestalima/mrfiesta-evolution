...
export function CelebrationWhatsAppCTA({ title, variant = 'compact', className = '' }: { title: string; variant?: 'compact' | 'modal'; className?: string }) {
  const label = 'Quiero una fiesta así, abrir conversación en WhatsApp'
  const content: ReactNode = <><WhatsAppMark/><span className="whatsapp-cta-copy"><strong>QUIERO UNA FIESTA ASÍ</strong><small>ABRIR EN WHATSAPP</small></span><ArrowUpRight className="whatsapp-cta-arrow" size={variant === 'modal' ? 18 : 15}/></>
